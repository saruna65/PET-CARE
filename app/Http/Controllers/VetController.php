...
class VetController extends Controller
{
    ...
    public function registerForm(): View|RedirectResponse
    {
        ...
        if (Auth::user()->isVet() && Auth::user()->vetProfile) {
            return redirect()->route('vet.index')->with('info', 'You are already registered as a veterinarian.');
        }
    ...
    public function registerSubmit(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('vet.index')->with('success', 'Your veterinarian profile has been created! It will be reviewed by our team shortly.');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()
                ->with('error', 'There was a problem creating your vet profile: ' . $e->getMessage())
                ->withInput();
        }
    }
    ...
    public function getDiseaseDetails(string $id)
    {
        ...
        $pet = $disease->pet;
        $petImage = $pet && $pet->image_path ? asset('storage/' . $pet->image_path) : asset('images/default-pet.png');
    ...
    public function updateProfile(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'experience_years' => 'required|integer|min:0|max:70',
            'qualification' => 'required|string|max:255',
            'license_number' => ['required', 'string', 'max:50', Rule::unique('vets')->ignore($vet->id)],
            'biography' => 'nullable|string',
            'services_offered' => 'nullable|array',
            'address' => 'nullable|string|max:255',
            'city' => 'nullable|string|max:100',
        ...
        return redirect()->route('vet.dashboard')->with('success', 'Veterinarian profile updated successfully!');
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('vet.index')->with('success', 'Veterinarian registered successfully!');
    }
    ...
    public function update(Request $request, string $id): RedirectResponse
    {
        ...
        $validated = $request->validate([
            'clinic_name' => 'required|string|max:255',
            'specialization' => 'required|string|max:255',
            'license_number' => ['required', 'string', 'max:50', Rule::unique('vets')->ignore($id)],
            'experience_years' => 'nullable|integer|min:0',
            'qualification' => 'nullable|string|max:255',
            'phone_number' => 'nullable|string|max:20',
            'is_available' => 'boolean',
        ...
        return redirect()->route('vet.index')->with('success', 'Veterinarian updated successfully!');
    }
    ...
    public function destroy(string $id): RedirectResponse
    {
        ...
        return redirect()->route('vet.index')->with('success', 'Veterinarian removed successfully!');
    }
    ...
    public function storeWithUser(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('vet.index')->with('success', 'New veterinarian registered successfully with a user account!');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()
                ->with('error', 'There was a problem creating the vet with user account: ' . $e->getMessage())
    ...
    public function submitReview(Request $request, string $id): RedirectResponse
    {
        ...
        if ($disease->is_reviewed) {
            return redirect()->route('vet.dashboard')->with('warning', 'This case has already been reviewed.');
        }
        ...
        'is_critical' => 'sometimes|nullable|boolean',
        'has_zoonotic_risk' => 'sometimes|nullable|boolean',
        'zoonotic_precautions' => 'required_if:has_zoonotic_risk,1|nullable|string',
    ]);
        ...
        'vet_notes' => $validated['notes'] ?? null,
        // Use the proper way to handle checkboxes with boolean values
        'is_critical' => $request->has('is_critical'),
        'has_zoonotic_risk' => $request->has('has_zoonotic_risk'),
        'zoonotic_precautions' => $validated['zoonotic_precautions'] ?? null,
        'is_reviewed' => true,
        'reviewed_at' => now(),
        'reviewed_by' => auth()->id(),
    ]);
        ...
        return redirect()->route('vet.dashboard')->with('success', 'Your review has been submitted successfully.');
    }
}
