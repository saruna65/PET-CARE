<x-app-layout><!-- vet review -->
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('vet.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg transition duration-300">
                        ...
                        <div class="flex items-center">
                            <div class="h-16 w-16 rounded-full overflow-hidden mr-4">
                                @if($pet && $pet->image_path)
                                    <img src="{{ asset('storage/' . $pet->image_path) }}" alt="{{ $pet->pet_name }}" class="h-full w-full object-cover">
                                @else
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 mt-1">Owner: {{ $disease->user ? $disease->user->name : 'Unknown Owner' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <div class="bg-gray-100 rounded-xl overflow-hidden shadow-md">
                                <img src="{{ asset('storage/' . $disease->image_path) }}" alt="Uploaded condition" class="w-full h-auto max-h-96 object-contain">
                            </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200">
                        ...
                        @if($disease->is_reviewed && $disease->vet_diagnosis)
                            ...
                            <div class="bg-green-50 rounded-lg p-6 mb-6">
                                ...
                                <div class="mt-4 pt-4 border-t border-green-200 text-sm text-gray-600 flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                                    </svg>
                        ...
                        @else
                            ...
                            <form action="{{ route('vet.disease.submit-review', $disease->id) }}" method="POST" class="bg-gray-50 rounded-lg p-6">
                                @csrf
                                
                                <div class="mb-6">
                                    <label for="diagnosis" class="block text-sm font-medium text-gray-700 mb-1">Diagnosis</label>
                                    <input type="text" name="diagnosis" id="diagnosis" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-500 focus:ring-opacity-50" value="{{ old('diagnosis') }}" required>
                                    @error('diagnosis')
                                ...
                                <div class="mb-6">
                                    <label for="treatment" class="block text-sm font-medium text-gray-700 mb-1">Recommended Treatment</label>
                                    <textarea name="treatment" id="treatment" rows="5" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-500 focus:ring-opacity-50" required>{{ old('treatment') }}</textarea>
                                    @error('treatment')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea name="notes" id="notes" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-500 focus:ring-opacity-50">{{ old('notes') }}</textarea>
                                    @error('notes')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-6">
                                    <div class="flex items-center">
                                        <input type="checkbox" name="is_critical" id="is_critical" class="rounded border-gray-300 text-amber-600 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-500 focus:ring-opacity-50" {{ old('is_critical') ? 'checked' : '' }}>
                                        <label for="is_critical" class="ml-2 text-sm font-medium text-gray-700">Mark as Critical Case</label>
                                    </div>
                                    <p class="text-sm text-gray-500 mt-1">Check this if the condition requires immediate attention or is potentially life-threatening.</p>
                                </div>
                                ...
                                <div class="flex justify-end space-x-4">
                                    ...
                                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg shadow transition duration-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
